...
if __name__ == "__main__":
    ...
    store_path = Path(EMBEDDING_DATA_PATH)
    if not store_path.exists():
        store_path.mkdir()
    e_store = EmbeddingStore(store_path)

    with open('{}_{}.csv'.format(EMBEDDING_ENTITY, EMBEDDING_MODEL_TYPE), 'w') as csvfile:
        ...
        for p_id, n1_data, n2_data, scores in tqdm(reader.get_news_data()):
            ...
            n1_doc = nlp(n1_data.title, disable=["tok2vec", "tagger", "parser", "attribute_ruler", "lemmatizer"])
            n2_doc = nlp(n2_data.title, disable=["tok2vec", "tagger", "parser", "attribute_ruler", "lemmatizer"])
            n1_sents, n2_sents = [sent.text for sent in n1_doc.sents], [sent.text for sent in n2_doc.sents]
            if len(n2_sents) > 0 and len(n1_sents) > 0:
                ...
                log.error(f"{p_id} does not have title. \nT1: {n1_data.title}\nT2: {n2_data.title}")